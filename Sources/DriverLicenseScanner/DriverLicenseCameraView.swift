//
//  DriverLicenseCameraView.swift
//  DriverLicenseScannerPackageDescription
//
...
protocol DriverLicenseCameraViewDelegate: AnyObject {
  func didCapture(image: CGImage)
  func didError(with: DriverLicenseScannerError)
}

final class DriverLicenseCameraView: UIView {
  weak var delegate: DriverLicenseCameraViewDelegate?
  
  private let driverLicenseFrameStrokeColor: UIColor
  ...
  private let captureSessionQueue = DispatchQueue(
    label: "com.yhkaplan.driver-license-scanner.captureSessionQueue"
  )
  
  // MARK: - Capture related
  
  private let sampleBufferQueue = DispatchQueue(
    label: "com.yhkaplan.driver-license-scanner.sampleBufferQueue"
  )
  
  init(
    delegate: DriverLicenseCameraViewDelegate,
    driverLicenseFrameStrokeColor: UIColor,
    maskLayerColor: UIColor,
    maskLayerAlpha: CGFloat
  ) {
    self.delegate = delegate
    self.driverLicenseFrameStrokeColor = driverLicenseFrameStrokeColor
  ...
  private let imageRatio: DriverLicenseImageRatio = .vga640x480
  ...
  private func _setupCamera() {
    ...
    guard let videoDevice = AVCaptureDevice.default(.builtInWideAngleCamera,
                                                    for: .video,
                                                    position: .back) else {
      delegate?.didError(with: DriverLicenseScannerError(kind: .cameraSetup))
      return
    }
    do {
      ...
      delegate?.didError(with: DriverLicenseScannerError(kind: .cameraSetup, underlyingError: error))
    }
  ...
  func setupRegionOfInterest() {
    ...
    let cuttedHeight: CGFloat = cuttedWidth * DriverLicense.heightRatioAgainstWidth
    ...
    strokeLayer.strokeColor = driverLicenseFrameStrokeColor.cgColor
    ...
    let interestHeight = interestWidth * DriverLicense.heightRatioAgainstWidth
...
extension DriverLicenseCameraView: AVCaptureVideoDataOutputSampleBufferDelegate {
  func captureOutput(_ output: AVCaptureOutput, didOutput sampleBuffer: CMSampleBuffer, from connection: AVCaptureConnection) {
    semaphore.wait()
    defer { semaphore.signal() }
    guard let pixelBuffer = CMSampleBufferGetImageBuffer(sampleBuffer) else {
      delegate?.didError(with: DriverLicenseScannerError(kind: .capture))
      delegate = nil
      return
    }
    var cgImage: CGImage?
...
extension DriverLicense {
  ...
  static let heightRatioAgainstWidth: CGFloat = 0.6353
}
